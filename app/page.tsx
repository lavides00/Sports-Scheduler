...
export default function Home() {
  ...
  async function book() {
    ...
    const r = await fetch("/api/bookings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bookingDate: dateKey, courtId: selected.court.id, sport: selected.court.sport, startTime: selected.time, endTime: end, customerName: name }) });
    const data = (await r.json()) as BookingResponse;
    if (!r.ok) { setNotice(data.error || "Could not book this slot."); return; }
    setBookings(prev => [...prev, { ...data, court_id: data.courtId, start_time: data.startTime, customer_name: data.customerName }]);
